...
- 共享层 `Shared/`（双 target 成员）：`SharedAudioRing`（mmap 单生产者单消费者环形缓冲，
  ...
  Darwin 通知，跨进程纯唤醒）、`BroadcastShared`（规范 PCM 格式、通知名，及**运行时派生**的
  标识符）。
- **标识符不写死签名身份**：App Group / 扩展 id / 通知名都由 `Bundle.main.bundleIdentifier`
  在运行时派生（扩展进程剥掉 `.BroadcastExtension` 后缀即得主 App id，两端算出同值）。
  entitlements 用构建变量展开——主 App `group.$(PRODUCT_BUNDLE_IDENTIFIER)`、扩展
  `group.$(PRODUCT_BUNDLE_IDENTIFIER:base)`（`:base` 恰好剥掉 `.BroadcastExtension`），二者都
  解析为 `group.<主App id>`。故提交到仓库的源码/entitlements **不含任何 bundle id / team /
  App Group 字面量**；个人 signing 仅存在于本地未提交的 pbxproj。
- 扩展 `LumaBroadcastExtension/`：`SampleHandler: RPBroadcastSampleHandler` 只取
  ...
  = RPBroadcastProcessModeSampleBuffer`（缺则被 ASC 拒）。
- 主 App：`BroadcastAudioProvider`（`AppDependencies.makeAudioProvider` 的 iOS `.systemAudio`
  ...
  （Start 失败则回收）。`BroadcastStateMonitor` 观察扩展的 Darwin 通知，控制栏显示
  「等待广播 / 正在广播」徽标。
- **Ring 文件生命周期**：ring 文件常驻 App Group（1 MiB，从不 unlink——扩展可能仍 mmap 着
  它，unlink 会把那份映射困在孤儿 inode 上、音频静默丢失）。会话 start 在**原 inode 上**
  就地复用：头有效则保留单调游标、把 read 快进到 write 丢弃积压；头无效才重新初始化。
  打开方（扩展）校验 magic/version/capacity 与文件大小后才信任映射。
...
用户从控制中心停广播 → provider 结束音频流 → `SessionController` 在事件流自然结束后
自动走 stop 收尾回 idle（扩展进程被杀而没发 finished 通知的场景仍无自动收尾，属已知边角）。
